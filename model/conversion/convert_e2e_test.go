...
func TestBackAndForthConversion(t *testing.T) {
	discoveredDevice := ConvertToDiscoveredDevice(getTestDevice())
	transformedDevice := TransformDevice(discoveredDevice, "URI", "v0.9.0")
...
func getTestDevice() *model.DeviceInfo {
	manufacturer := "Siemens AG"
	product := "TestDevice"
	serialNumber := fmt.Sprint(lastSerialNumber.Load())
	deviceInfo := model.NewDevice("Asset", "TestDevice")
