...
# Example: al-ctl test api -e localhost:8081 --service-name identifiers -p misc/identifier_request.json -v --base-schema-path model/iah_base_v1.11.0.yaml --target-class Asset
```
...
- The `iah_base_v1.11.0.yaml` file is used as the base schema for validation, which can be found in the [model](https://github.com/industrial-asset-hub/asset-link-sdk/tree/main/model) directory.
